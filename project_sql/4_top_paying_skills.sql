...
1. SELECT Clause: This specifies the columns to be retrieved. Its includes skills_dim.skills (the skill name) and ROUND(AVG(job_postings_fact.salary_year_avg), 0), which calculates the average yearly salary associated with each skill and rounds the result to the nearest whole number.
